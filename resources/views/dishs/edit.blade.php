            ...
            <form method="post" action="/dishs/{{$dish->id}}?ref={{urlencode($ref)}}" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="form-group col-md-6 col-lg-4">
                        ...
                        <input readonly id="dish_id" name="id" class="form-control form-control-sm" value="{{old('id', $dish->id)}}" type="number" required />
                        @error('id')<span class="text-danger">{{$message}}</span>@enderror
                    </div>
                    <div class="form-group col-md-6 col-lg-4">
                        <label for="dish_name">Name</label>
                        <input id="dish_name" name="name" class="form-control form-control-sm" value="{{old('name', $dish->name)}}" required maxlength="50" />
                        @error('name')<span class="text-danger">{{$message}}</span>@enderror
                    </div>
                    <div class="form-group col-md-6 col-lg-4">
                        <label for="dish_price">Price</label>
                        <input id="dish_price" name="price" class="form-control form-control-sm" value="{{old('price', $dish->price)}}" type="number" step="0.1" required />
                    ...
                    <div class="form-group col-md-6 col-lg-4">
                        ...
                        <a href="/storage/dishs/{{old('image', $dish->image)}}" target="_blank" title="{{old('image', $dish->image)}}"><img class="img-item" src="/storage/dishs/{{old('image', $dish->image)}}" /></a>
